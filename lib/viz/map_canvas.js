...
MapCanvas = function(map, canvas, params) {
  ...
  this.draw(this.map, this.buffer, this.color_theme);
...
HighlightCanvas.prototype.drawSelect = function(ids, ctx, color_theme) {
  var map = this.map,
      color_dict = {};
  ...
  var screenObjs = map.screenObjects;
  var shpType = map.shpType.toUpperCase();
...
HighlightCanvas.prototype.highlight = function(ids, ctx, nolinking) {
  ...
  if (ids.length > 0) {
    ...
    this.drawSelect( ids, ctx, this.color_theme);
...
HighlightCanvas.prototype.highlightExt = function(ids, extent, linking) {
  ...
  var x0 = extent[0],
      ...
      x1 = extent[2],
      y1 = extent[3];
  var pt0 = this.mapToScreen(x0, y0),
      pt1 = this.mapToScreen(x1, y1);
  var startX = pt0[0],
      startY = pt0[1],
      w = pt1[0] - startX,
      h = pt1[1] - startY;
  ...
  if ( this.selected.length > 0 ) {
    ...
    if (this.shpType != "POINT" && this.shpType != "MULTIPOINT") {
      ...
      this.drawSelect(ddraw, ctx, this.color_theme);
      this.drawSelect(hdraw, ctx, this.color_theme);
  ...
  function OnMouseUp(evt) {
    ...
    if ( isMouseDown == true) {
      ...
      if ( self.startX == x && self.startY == y ) {
        ...
        x = pt[0];
        y = pt[1];
        for ( var i=0, n=self.bbox.length; i<n; i++ ) {
          var box = self.bbox[i];
          if ( x > box[0] && x < box[1] &&
               y > box[2] && y < box[3] ) {
...
var MapCanvas = function(map, canvas, hlcanvas, params) {
  ...
  this.SetupHighlightCanvas(this.hlcanvas, this);
  ...
  this.draw(map, this.buffer, this.color_theme);
...
MapCanvas.prototype = {
  ...
  SetupHighlightCanvas : function(canvasObj, parent, map) {
    canvasObj.map = parent.map;
    canvasObj.width = parent.width;
    canvasObj.height = parent.height;
    canvasObj.buffer = parent.buffer;
    canvasObj.noForeground = parent.noForeground;
    canvasObj.STROKE_WIDTH = parent.STROKE_WIDTH;
    canvasObj.STROKE_CLR = parent.STROKE_CLR;
    canvasObj.HL_ALPHA = parent.HL_ALPHA;
    canvasObj.parent = parent;

    // internal properties for mouse events
    canvasObj.selected = [];
    canvasObj.brushRect = undefined; //GRect object
    canvasObj.isBrushing = false;
    canvasObj.startX = -1;
    canvasObj.startY = -1;
    canvasObj.currentX = -1;
    canvasObj.currentY = -1;
    canvasObj.startPX = undefined;
    canvasObj.startPY = undefined;
    canvasObj.isMouseDown = false;
    canvasObj.isMouseUp = false;
    canvasObj.isMouseMove = false;
    canvasObj.isKeyDown = false;


    canvasObj.bbox = parent.map.bbox;
    canvasObj.screenToMap = parent.map.screenToMap;
    canvasObj.highlightExt = parent.highlightExt;
    canvasObj.highlight = parent.highlight;
    canvasObj.fadeCanvas = parent.fadeCanvas; // used in highlight()
    canvasObj.clearCanvas = parent.clearCanvas;
    canvasObj.resetDraw = parent.resetDraw;
    canvasObj.triggerLink = parent.triggerLink;

    canvasObj.CheckBrush = function(x,y) {
      var brushRect = this.brushRect;
      if (brushRect == undefined ||
          (brushRect && !brushRect.Contains(x, y)) ) {
        brushRect = undefined;
        this.isBrushing = false;
      }
      if (brushRect && brushRect.Contains(x, y) ) {
        this.isBrushing = true;
      }
    };

    canvasObj.addEventListener('touchmove', parent.OnMouseMove, false);
    canvasObj.addEventListener('touchstart', parent.OnMouseDown, false);
    canvasObj.addEventListener('touchend', parent.OnMouseUp, false);

    canvasObj.addEventListener('mousemove', parent.OnMouseMove, false);
    canvasObj.addEventListener('mousedown', parent.OnMouseDown, false);
    canvasObj.addEventListener('mouseup', parent.OnMouseUp, false);
  },
  ...
  highlight: function( ids, ctx, nolinking ) {
    ...
    if ( ctx == undefined) {
      ...
      if (!this.noForeground) {
        ...
        ctx.drawImage( this.parent.buffer, 0, 0);
        ctx.globalAlpha = 1;
      }
    }
    ...
    if (ids.length > 0) {
      ctx.lineWidth = this.STROKE_WIDTH;
      ctx.strokeStyle = this.STROKE_CLR;
      this.parent.drawSelect( ids, ctx, this.color_theme);
  ...
  highlightExt: function( ids, extent, linking) {
    ...
    if ( this.selected.length > 0 ) {
      ...
      // fade the current canvas-es
      this.fadeCanvas(this.parent.canvas, true);
      ...
      if (this.shpType != "POINT" && this.shpType != "MULTIPOINT") {
        ...
        ctx.fillStyle = "rgba(0,0,0,1)";
        ctx.globalCompositeOperation = "destination-out";
        this.parent.drawSelect(ddraw, ctx, this.color_theme);
        this.parent.drawSelect(hdraw, ctx, this.color_theme);
  ...
  fadeCanvas : function(can, is_highlight) {
    /*var el = obj,
        nn = el.parentNode.children(),
        idx = nn.length - 1,
        can = nn[idx];
    if ($(can).is("canvas")) {
    */
  ...
  // draw map to a canvas with colors/fillColor/strokeColor/lineWidth
  draw: function(canvasObj,  colors, fillColor, strokeColor, lineWidth) {
    fillColor = fillColor ? fillColor : this.FILL_CLR;
    lineWidth = lineWidth ? lineWidth: this.STROKE_WIDTH;
    strokeColor =  strokeColor ? strokeColor : this.STROKE_CLR;

    var map = this.map,
        shpType = map.shpType.toUpperCase(),
  ...
  drawSelect: function(ids, ctx, color_theme) {
    var map = this.map,
        color_dict = {};
  ...
  // register mouse events of highlight canvas
  ...
  OnMouseMove: function(evt) {
    var obj = evt.target;
    if ( obj.isMouseDown == true ) {
      if (evt.type.indexOf("touch")>=0) {
        if (evt.touches.length === 1) {
          obj.currentX = evt.targetTouches[0].pageX;
          obj.currentY = evt.targetTouches[0].pageY -48;
        } else
            return;
      } else {
        obj.currentX = evt.offsetX;
        obj.currentY = evt.offsetY;
      }
      ...
      if ( obj.isBrushing == true ) {
        var offsetX = currentX - startX,
            offsetY = currentY - startY;
        x0 = obj.brushRect.x0 + offsetX;
        x1 = obj.brushRect.x1 + offsetX;
        y0 = obj.brushRect.y0 + offsetY;
        y1 = obj.brushRect.y1 + offsetY;
      }
      var pt0 = obj.screenToMap(x0, y0),
          pt1 = obj.screenToMap(x1, y1);
      ...
      obj.highlightExt([], [x0,y0,x1,y1], true);
    }
    evt.preventDefault();
  },
  ...
  OnMouseUp: function(evt) {
    ...
    if ( obj.isMouseDown == true) {
      ...
      if ( obj.startX == x && obj.startY == y ) {
        // point select
        obj.clearCanvas(obj); // clear it self
        var pt = obj.screenToMap(x, y);
        x = pt[0];
        y = pt[1];
        for ( var i=0, n=obj.bbox.length; i<n; i++ ) {
          var box = obj.bbox[i];
          if ( x > box[0] && x < box[1] &&
               y > box[2] && y < box[3] ) {
            obj.isKeyDown = false;
            obj.isBrushing = false;
            obj.brushRect = undefined;
            obj.highlight([i]);
        ...
        obj.resetDraw();
        obj.triggerLink([], undefined);

      } else if ( obj.isBrushing == false) {
        // setup brushing box
        obj.brushRect = new GRect( obj.startX, obj.startY, x, y);
      }
    }
    obj.isMouseDown = false;
    evt.preventDefault();
