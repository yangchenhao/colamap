...
var MapCanvas = function(map, canvas, hlcanvas, params) {
  ...
  this.setupCanvas(this.hlcanvas, this);
...
MapCanvas.prototype.setupCanvas = function (canvasObj, parent) {
  ...
  canvasObj.fadeCanvas = parent.triggerLink;
...
MapCanvas.prototype = {
  ...
  OnMouseMove: function(evt) {
    ...
    if ( obj.isMouseDown == true ) {
      ...
      var pt0 = obj.map.screenToMap(x0, y0),
          pt1 = obj.map.screenToMap(x1, y1);
  ...
  OnMouseUp: function(evt) {
    ...
    if ( obj.isMouseDown == true) {
      ...
      if ( obj.startX == x && obj.startY == y ) {
        // point select
        obj.clearCanvas(obj.hlcanvas);
        var pt = obj.map.screenToMap(x, y);
        x = pt[0];
        y = pt[1];
        for ( var i=0, n=obj.map.bbox.length; i<n; i++ ) {
          var box = obj.map.bbox[i];
          if ( x > box[0] && x < box[1] &&
               y > box[2] && y < box[3] ) {
            obj.isKeyDown = false;
